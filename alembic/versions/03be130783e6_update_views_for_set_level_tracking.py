"""update_views_for_set_level_tracking

Revision ID: 03be130783e6
Revises: ed37c08af829
Create Date: 2025-12-17 01:48:50.726639+00:00
...
revision = '03be130783e6'
down_revision = 'ed37c08af829'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    # ==============================================
    # Updated to use log_sets table - tracks max weight per set
    op.execute("""
...
def downgrade() -> None:
    ...
    # Recreate original views (using aggregate columns from log_exercises)
    # These will work after the first migration downgrade restores the columns

    # 1. USER EXERCISE PROGRESS VIEW (original)
    op.execute("""
        CREATE MATERIALIZED VIEW user_exercise_progress_weekly AS
        SELECT
            ws.user_id,
    ...
    # 2. WORKOUT VOLUME BY WEEK VIEW (original)
    op.execute("""
        CREATE MATERIALIZED VIEW workout_volume_by_week AS
        SELECT
            ws.user_id,
    ...
    # 3. WORKOUT VOLUME BY MONTH VIEW (original)
    op.execute("""
        CREATE MATERIALIZED VIEW workout_volume_by_month AS
        SELECT
            ws.user_id,
    ...
    # 4. PERSONAL RECORDS VIEW (original)
    op.execute("""
        CREATE MATERIALIZED VIEW personal_records AS
        WITH ranked_lifts AS (
            SELECT
    ...
    # 5. MUSCLE GROUP DISTRIBUTION VIEW (original)
    op.execute("""
        CREATE VIEW muscle_group_distribution AS
        SELECT
            ws.user_id,
    ...
    # 6. USER WORKOUT SUMMARY VIEW (original)
    op.execute("""
        CREATE VIEW user_workout_summary AS
        SELECT
            ws.user_id,
    ...
    # 7. EXERCISE FREQUENCY VIEW (original)
    op.execute("""
        CREATE VIEW exercise_frequency AS
        SELECT
            ws.user_id,
